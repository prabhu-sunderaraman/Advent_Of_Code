...
var testInput string = `1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc`

var puzzleInput string = `2-6 c: fcpwjqhcgtffzlbj
6-9 x: xxxtwlxxx
5-6 w: wwwwlwwwh
...
func TestFindNumberOfValidPasswordsCountForTestInput(t *testing.T) {
	passwords := strings.Split(testInput, "\n")
	count := FindNumberOfValidPasswords(passwords)
	if count != 2 {
		t.Error("expected 2 valid passwords", count)
	}
}

func TestFindNumberOfValidPasswordsCountForTestInputPartB(t *testing.T) {
	passwords := strings.Split(testInput, "\n")
	count := FindNumberOfValidPasswordsBasedOnPositions(passwords)
	if count != 1 {
		t.Error("expected 1 valid password", count)
	}
}

func TestFindNumberOfValidPasswordsCountForPuzzleInputPartB(t *testing.T) {
	passwords := strings.Split(puzzleInput, "\n")
	count := FindNumberOfValidPasswordsBasedOnPositions(passwords)
	if count != 729 {
		t.Error("expected 729 valid passwords", count)
	}
}

func TestFindNumberOfValidPasswordsCountForPuzzleInput(t *testing.T) {
	passwords := strings.Split(puzzleInput, "\n")
	count := FindNumberOfValidPasswords(passwords)
	if count != 582 {
		t.Error("expected 582 valid passwords", count)
	}
}
