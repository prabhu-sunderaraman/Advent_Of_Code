...
tmzxl (23)
"""
data class Tower(val name:String, val weight: Int?, var subTowers:MutableList<Tower>,var occurences:Int,var totalWeight:Int=0)
val resultMap = mutableMapOf<String,Tower>()
var aggTower = null 
fun main(args:Array<String>):Unit{
		val lines = input.trim().split("\n")
		val linesWithoutNesting = lines.filter {line -> !line.contains("->")}
		val linesWithNesting = lines.filter {line -> line.contains("->")}
		linesWithoutNesting.forEach{ line ->
			val code = line.split(" ")[0]
			val weight = line.split(" ")[1].trim().replace("(","").replace(")","").toInt()
			val value:Int = getCodeValue(code.trim())
			resultMap.put(code.trim(),Tower(code,weight,mutableListOf(),value))
		}
		linesWithNesting.forEach{ line ->
			val lhsCode = line.split("->")[0].trim()
			val code1 = lhsCode.split(" ")[0]
			val weight1 = lhsCode.split(" ")[1].trim().replace("(","").replace(")","").toInt()
			val value1:Int = getCodeValue(code1.trim())
			resultMap.put(code1.trim(),Tower(code1,weight1,mutableListOf(),value1))
		}
		linesWithNesting.forEach{ line ->
			val lhsCode = line.split("->")[0].trim()
			val code1 = lhsCode.split(" ")[0]
			val rhsCode = line.split("->")[1].trim()
			val rhsCodeArr = rhsCode.split(",")
			rhsCodeArr.forEach { code2 ->
				val value2:Int = getCodeValue(code2.trim())
				if(value2 > 1){
					val tower = resultMap[code2.trim()]
					tower?.occurences = value2
					val nestedTower = resultMap[code1.trim()]
					val weight:Int? = resultMap[code2.trim()]?.weight
					nestedTower?.subTowers?.add(Tower(code2.trim(),weight,mutableListOf(),1))
				}
			}
		}
		val finalTower:Tower? = resultMap.values.firstOrNull { tower -> tower.occurences == 1}
		if(finalTower != null){
			println("Bottom program is ${finalTower.name}")
			aggregateResultMap(finalTower)
			calculateTotalWeight(finalTower)
			findUnbalancedProgram(finalTower)
		}
}

fun calculateTotalWeight(tower:Tower):Unit{
	var totalWeight = tower.weight!!
	tower.subTowers.forEach { subTower -> 
		calculateTotalWeight(subTower)
		totalWeight += subTower.totalWeight
	}
	tower.totalWeight = totalWeight
}

fun aggregateResultMap(tower:Tower){
	if(tower.subTowers.size != 0){
		val subLevelTowers = mutableListOf<Tower>()
		tower.subTowers.forEach{ subTower -> 
			subLevelTowers.add(resultMap[subTower.name]!!)
		}
		tower.subTowers = subLevelTowers
		tower.subTowers.forEach{ subTower -> 
			aggregateResultMap(subTower)
		}
	}
}

fun findUnbalancedProgram(tower:Tower){
	var towerWeightCountMap = mutableMapOf<Int,Int>()
	tower.subTowers.forEach {subTower ->
		if(towerWeightCountMap[subTower.totalWeight] == null)
			towerWeightCountMap[subTower.totalWeight] = 1
		else
			towerWeightCountMap[subTower.totalWeight] =  towerWeightCountMap[subTower.totalWeight]!! + 1	
	}
	val unbalancedItem = towerWeightCountMap.filter { (_,value) -> value == 1 }
	val balancedItem = towerWeightCountMap.filter { (_,value) -> value != 1 }
	var unbalancedTotalWeight = 0
	for( (key,_) in unbalancedItem)
		unbalancedTotalWeight = key
	var balancedTotalWeight = 0
	for( (key,_) in balancedItem)
		balancedTotalWeight = key	
		
	val unbalancedTower:Tower? = tower.subTowers.find { subTower -> subTower.totalWeight == unbalancedTotalWeight }
	
	println("Balanced Weight: ${balancedTotalWeight}, Unbalanced Weight: ${unbalancedTotalWeight}, Unbalanced Program: ${unbalancedTower?.name}")

	printBalancedWeight(unbalancedTower,(unbalancedTotalWeight - balancedTotalWeight))
}

fun printBalancedWeight(unbalancedTower:Tower?,differenceInWeightFactor:Int):Unit{
	if(unbalancedTower != null){
		var towerWeightCountMap = mutableMapOf<Int,Int>()
		unbalancedTower.subTowers.forEach { subTower -> 
			if(towerWeightCountMap[subTower.totalWeight] == null)
				towerWeightCountMap[subTower.totalWeight] = 1
			else
				towerWeightCountMap[subTower.totalWeight] =  towerWeightCountMap[subTower.totalWeight]!! + 1
		...
		for( (key,_) in unbalancedItem)
			unbalancedTotalWeight = key
		var balancedTotalWeight2 = 0
		for( (key,_) in balancedItem)
			balancedTotalWeight2 = key	
		if(unbalancedTotalWeight != 0){
			val unbalancedTower2:Tower? = unbalancedTower.subTowers.find { subTower -> subTower.totalWeight == unbalancedTotalWeight }
			printBalancedWeight(unbalancedTower2,differenceInWeightFactor)
		}
		else{
				println("Balanced Weight: ${balancedTotalWeight2}, Unbalanced Weight: ${unbalancedTower.weight}, Unbalanced Program: ${unbalancedTower.name}")
				val retValue = unbalancedTower.weight!! - differenceInWeightFactor
				println("Weight that should be changed to balance is ${retValue}")
		}	
	}
}

fun printBottom(){
	resultMap.forEach {(item,value) -> if(value.occurences > 1) println("${item}: ${value.occurences}")}
}
fun getCodeValue(code:String):Int{
	var value = 1
	if(resultMap[code] != null){
		val existingValue = resultMap[code]?.occurences
