...
let maxRegisterValueInIteration = 0
let maxRegisterInIteration = 0

inputLines.forEach(inputLine => {
	let lineArr = inputLine.split(" ")
	let register = lineArr[0]
	let registerValue = registers.has(register) ? registers.get(register) : 0
	let operation = lineArr[1]
	let operand = parseInt(lineArr[2])
	let conditionRegister = lineArr[4]
	let conditionRegisterValue = registers.has(conditionRegister) ? registers.get(conditionRegister) : 0
	let conditionOperator = lineArr[5]
	let conditionOperand = parseInt(lineArr[6])	
	
	let operationAllowed = false
	if(conditionOperator == ">"){
		if(conditionRegisterValue > conditionOperand)
	...
	if(operationAllowed){
		...
		registers.set(register,registerValue)
		if(registerValue >= maxRegisterValueInIteration){
			maxRegisterValueInIteration = registerValue
			maxRegisterInIteration = register
		}
	}
})
console.log(registers);
let maxRegister = "NONE"
let maxRegisterValue = 0
for(let key of registers.keys()){
	if(registers.get(key) >= maxRegisterValue){
		maxRegisterValue = registers.get(key)
		maxRegister = key
	}	
}
console.log(maxRegister,maxRegisterValue);
console.log(maxRegisterInIteration,maxRegisterValueInIteration);
