...
let isAnagram = function(a,b){
	...
	if(a.length == b.length){
		...
		for(let i=0;i<bArr.length;i++){
			if(anagArr.includes(bArr[i])){
			   const index = anagArr.indexOf(bArr[i]);
			   anagArr.splice(index, 1);
			}
			else{
				valid = false
				break
...
let isPassphraseValid = function(passphrase){
	let originalArr = passphrase.split(' ')
	let valid = true
	for(let i=0;i<originalArr.length;i++){
		for(let j=i+1;j<originalArr.length;j++){
			if(isAnagram(originalArr[i],originalArr[j])){
				valid = false
				break
			}
		}
...
let findTotal = function(passphraseCollection){
	let count = 0
	let collection = passphraseCollection.split("\n")
	collection.forEach(it => {
		if(it !== ''){
			if(isPassphraseValid(it.trim()))
				count++
		}
	})
	return count
